...
class SignupScene extends SceneBuilder {
  ...
  void setLayout() {
    ...
    Css.setButton(700, 25, 20, signUpButton, logInButton);
    Css.setTextField(700, 20, 17, usernameField, emailField, passwordField, confirmPasswordField);
    Css.setLabel(13, usernameLabel, emailLabel, passwordLabel, confirmPasswordLabel);
      ...
      try {
        if (feedback()) {
          if (Authentication.register(usernameField.getText(), passwordField.getText(), emailField.getText())) {
            ...
            Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: This username is already taken", 13, signupFeedbackLabel, 6);
        ...
        Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Could not connect to database", 13, signupFeedbackLabel, 6);
        loadingAnimation.setVisible(false);
  ...
  private boolean feedback() {
    ...
    if (passwordField.getText().trim().length() == 0 || usernameField.getText().trim().length() == 0 || emailField.getText().trim().length() == 0) {
      Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Password, username or email are missing", 13, signupFeedbackLabel, 6);
      return false;
    }
    //Checks if password is shorter than 8 characters
    if (passwordField.getText().length() < 8) {
      Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Password needs to contain 8 characters", 13, signupFeedbackLabel, 6);
      return false;
    }
    ...
    if (usernameField.getText().length() != 0) {
      for (int i = 0; i < usernameField.getText().length(); i++) {
        if (!Character.isLetter(usernameField.getText().charAt(i)) &&
          ...
          Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Username contains illegal character", 13, signupFeedbackLabel, 6);
          return false;
        }
      }
    }
    ...
    if (passwordField.getText().length() != 0) {
      for (int i = 0; i < passwordField.getText().length(); i++) {
        if (!Character.isLetter(passwordField.getText().charAt(i)) &&
          ...
          Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Password contains illegal character", 13, signupFeedbackLabel, 6);
          return false;
        }
      }
    }
    ...
    if (!passwordField.getText().equals(confirmPasswordField.getText())) {
      Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: Your passwords don't match", 13, signupFeedbackLabel, 6);
      return false;
    }
    //Checks if email contains "@" and "."
    if (!emailField.getText().contains("@") || !emailField.getText().contains(".")) {
      Css.playFeedBackLabelTransition(FeedBackType.ERROR, "Error: This email is not valid", 13, signupFeedbackLabel, 6);
      return false;
    }

    Css.playFeedBackLabelTransition(FeedBackType.SUCCESSFUL, "You have been signed up!", 13, signupFeedbackLabel, 6);
    return true;
  }
